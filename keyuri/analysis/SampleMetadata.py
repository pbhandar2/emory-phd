...
class SampleMetadata:
    ...
    def get_cache_feature_dict(
        ...
        for cache_size_ratio in cache_size_ratio_arr:
            cache_feature_dict["read_hr_{:.2f}".format(cache_size_ratio)] = rd_hist.get_read_hit_rate(int(wss_block*cache_size_ratio))
            cache_feature_dict["write_hr_{:.2f}".format(cache_size_ratio)] = rd_hist.get_write_hit_rate(int(wss_block*cache_size_ratio))
    ...
    def load_percent_diff_df(self) -> DataFrame:
        ...
        for percent_diff_file in sample_percent_diff_dir_path.iterdir():
            ...
            percent_diff_dict['mean_percent_read_hit_rate_error'] = self.get_mean_hit_rate_error(percent_diff_dict, "read")
            percent_diff_dict['mean_percent_write_hit_rate_error'] = self.get_mean_hit_rate_error(percent_diff_dict, "write")
            percent_diff_dict['mean_overall_error'] = mean([abs(percent_diff_dict['mean_percent_read_hit_rate_error']),
    ...
    @staticmethod
    def get_mean_hit_rate_error(percent_diff_dict, op):
        ...
        for feature_name in percent_diff_dict:
            if "{}_hr_".format(op) not in feature_name:
                continue 
